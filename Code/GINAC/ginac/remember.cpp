...
bool remember_table_entry::is_equal(const remember_table_entry& other) const
{
    GINAC_ASSERT(other.seq.size() == seq.size());
    if (other.hashvalue != hashvalue) return false;
    size_t num = seq.size();
    for (size_t i = 0; i < num; ++i)
        if (!seq[i].is_equal(other.seq[i])) return false;
    ++last_access = access_counter;
    ++successful_hits;
    return true;
}
