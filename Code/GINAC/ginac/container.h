 ...
 */
#pragma warning (disable : 4661) // !!!member function of template is not implemented, make sure they are
...
template <>
inline void container_storage<ginac_vector>::reserve(size_t n) { seq.reserve(n); }

template <>
inline void container_storage<ginac_vector>::reserve(ginac_vector<ex> & v, size_t n) { v.reserve(n); }
...
template<> inline unsigned container<std::list>::get_default_flags() { return status_flags::not_shareable; }

/** Specialization of container::get_open_delim() for lst. */
template<> inline char container<std::list>::get_open_delim() { return '{'; }

/** Specialization of container::get_close_delim() for lst. */
template<> inline char container<std::list>::get_close_delim() { return '}'; }

// defined in lst.cpp
template<> inline bool container<std::list>::info(unsigned inf) const
{
...
{
	ensure_if_modifiable();
    //push front is not a function of vector in STL
