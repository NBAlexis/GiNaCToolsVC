...
matrix::matrix(size_t r, size_t c) : row(r), col(c), m(r*c, _ex0)
{
	setflag(status_flags::not_shareable);
}
 ...
 *  thrown away. */
matrix::matrix(size_t r, size_t c, const lst & l)
...
matrix::matrix(size_t r, size_t c, const exvector & m2)
  : row(r), col(c), m(m2)
{
	setflag(status_flags::not_shareable);
}
matrix::matrix(size_t r, size_t c, exvector && m2)
...
void matrix::read_archive(const archive_node &n, lst &sym_lst)
{
    ...
    unsigned c = static_cast<unsigned>(col);

	if (!(n.find_unsigned("row", r)) || !(n.find_unsigned("col", c)))
		throw (std::runtime_error("unknown matrix dimensions in archive"));
	
    //r and c are updated in find unsigned
    row = r;
    col = c;
    m.reserve(row * col);
...
const ex & matrix::operator() (size_t ro, size_t co) const
{
	if (ro>=row || co>=col)
 ...
 *  @param co column of element
 *  @exception range_error (index out of range) */
ex & matrix::operator() (size_t ro, size_t co)
{
	if (ro>=row || co>=col)
		throw (std::range_error("matrix::operator(): index out of range"));
 ...
 */
int matrix::pivot(size_t ro, size_t co, bool symbolic)
{
	unsigned k = ro;
	if (symbolic) {
...
ex reduced_matrix(const matrix& m, unsigned r, unsigned c)
{
	...
	const size_t rows = m.rows()-1;
	const size_t cols = m.cols()-1;
	matrix & M = dynallocate<matrix>(rows, cols);
	M.setflag(status_flags::evaluated);
