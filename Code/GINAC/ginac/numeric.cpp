...
numeric::numeric(int i)
{
	...
	value = cln::cl_I(i);
#else
  #error "MinGW64 Should have cl_value_len >= 32"
...
numeric::numeric(long long numer, long long denom)
{
	if (!denom)
...
void numeric::do_print_csrc(const print_csrc & c, unsigned level) const
{
	...
	int oldprec = static_cast<int>(c.s.precision());
...
const numeric &numeric::operator=(long long i)
{
	return operator=(numeric(i));
}


const numeric &numeric::operator=(unsigned long long i)
{
	return operator=(numeric(i));
}
...
class lanczos_coeffs
{
	public:
		...
		int get_order() const { return static_cast<int>(current_vector->size()); }
...
cln::cl_N lanczos_coeffs::calc_lanczos_A(const cln::cl_N &x) const
{
	...
	int size = static_cast<int>(current_vector->size());
