...
class ginac_vector : public std::vector<T, _Alloc>
{
public:
    typedef std::vector<T, _Alloc> ginac_vector_base;
    typedef typename ginac_vector_base::size_type size_type;

    ginac_vector() : ginac_vector_base() {}
    ginac_vector(std::initializer_list<T> _Ilist, const _Alloc& _Al = allocator_type()) : ginac_vector_base(_Ilist, _Al) {}

    template<class _Iter, class = std::enable_if_t<std::_Is_iterator<_Iter>::value>>
    ginac_vector(_Iter _First, _Iter _Last, const _Alloc& _Al = _Alloc()) : ginac_vector_base(_First, _Last, _Al) {}

    ginac_vector(const size_type _Count, const _Alloc& _Al = _Alloc()) : ginac_vector_base(_Count, _Al) {}

    ginac_vector(const size_type _Count, const T& _Val, const _Alloc& _Al = _Alloc()) : ginac_vector_base(_Count, _Val, _Al) {}

    template<class _PT>
    void sort(_PT isless)
    {
        //https://stackoverflow.com/questions/4342957/why-does-vector-not-have-sort-method-as-a-member-function-of-vector-while-lis
        //sort is not a function of vector in STL
        //however, it says the above do the same thing
        std::sort(begin(), end(), isless);
    }
