...
class GINAC_API indexed : public exprseq
{
...
extern GINAC_API exvector get_all_dummy_indices(const ex & e);

/** More reliable version of the form. The former assumes that e is an
  * expanded expression. */
extern GINAC_API exvector get_all_dummy_indices_safely(const ex & e);
...
extern GINAC_API ex rename_dummy_indices_uniquely(exvector & va, const ex & b, bool modify_va = false);

/** Returns b with all dummy indices, which are common with a, renamed */
extern GINAC_API ex rename_dummy_indices_uniquely(const ex & a, const ex & b);

/** Same as above, where va and vb contain the indices of a and b and are sorted */
extern GINAC_API ex rename_dummy_indices_uniquely(const exvector & va, const exvector & vb, const ex & b);

/** Similar to above, where va and vb are the same and the return value is a list of two lists 
 *  for substitution in b */
extern GINAC_API lst rename_dummy_indices_uniquely(const exvector & va, const exvector & vb);
 ...
 */
extern GINAC_API ex expand_dummy_sum(const ex & e, bool subs_idx = false);
