 ...
 *  'is_equal'. */
class GINAC_API remember_table_entry {
public:
	remember_table_entry(function const & f, ex const & r);
	bool is_equal(function const & f) const;
    bool is_equal(const remember_table_entry& other) const;
    ...
    /* Those are required by STL */
    remember_table_entry() {}

    //it has is_equal but not has operator ==
    inline bool operator==(const remember_table_entry &other) const { return is_equal(other); }

    /* I'm NOT sure whether this is what they want!!! */
    inline bool operator<(const remember_table_entry &other) { return seq.size() < other.seq.size(); }
...
class GINAC_API remember_table_list : public std::list<remember_table_entry> {
public:

    /* Required by STL */
    remember_table_list() : max_assoc_size(0), remember_strategy(0) {}
	remember_table_list(unsigned as, unsigned strat) : max_assoc_size(as), remember_strategy(strat) {}
...
class GINAC_API remember_table : public std::vector<remember_table_list> {
public:
	remember_table();
