...
/** Symbolic matrices. */
class GINAC_API matrix : public basic
{
...
public:
	...
	matrix(size_t r, size_t c, const lst & l);
...
protected:
	matrix(size_t r, size_t c, const exvector & m2);
	matrix(size_t r, size_t c, exvector && m2);
...
public:
	...
	const ex & operator() (size_t ro, size_t co) const;
	ex & operator() (size_t ro, size_t co);
	matrix & set(size_t ro, size_t co, const ex & value) { (*this)(ro, co) = value; return *this; }
...
inline size_t rows(const matrix & m)
{ return m.rows(); }

inline size_t cols(const matrix & m)
{ return m.cols(); }
...
extern GINAC_API ex lst_to_matrix(const lst & l);

/** Convert list of diagonal elements to matrix. */
extern GINAC_API ex diag_matrix(const lst & l);
extern GINAC_API ex diag_matrix(std::initializer_list<ex> l);

/** Create an r times c unit matrix. */
extern GINAC_API ex unit_matrix(unsigned r, unsigned c);
...
extern GINAC_API ex symbolic_matrix(unsigned r, unsigned c, const std::string & base_name, const std::string & tex_base_name);
...
extern GINAC_API ex reduced_matrix(const matrix& m, unsigned r, unsigned c);

/** Return the nr times nc submatrix starting at position r, c of matrix m. */
extern GINAC_API ex sub_matrix(const matrix&m, unsigned r, unsigned nr, unsigned c, unsigned nc);
