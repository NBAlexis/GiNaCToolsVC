...
class GINAC_API G2_SERIAL { public: static unsigned serial; };
template<typename T1, typename T2>
...
extern GINAC_API ex lsolve(const ex &eqns, const ex &symbols, unsigned options = solve_algo::automatic);
...
extern GINAC_API const numeric fsolve(const ex& f, const symbol& x, const numeric& x1, const numeric& x2);
 ...
 */
extern GINAC_API ex convert_H_to_Li(const ex& parameterlst, const ex& arg);
