...
// x und y Integer -> bekannt.
	if (integerp(x)) {
		...
		if (integerp(y)) {
			...
			var cl_I_div_t q_r = floor2(x*d,c);
			var cl_I& q = q_r.quotient;
			var cl_I& r = q_r.remainder;
		...
		if (integerp(y)) {
			DeclareType(cl_I,y);
			var cl_I_div_t q_r = floor2(a,b*y);
			var cl_I& q = q_r.quotient;
			var cl_I& r = q_r.remainder;
			...
			var const cl_I& c = numerator(y);
			var const cl_I& d = denominator(y);
			var cl_I_div_t q_r = floor2(a*d,b*c);
			var cl_I& q = q_r.quotient;
			var cl_I& r = q_r.remainder;
...
#else
// Methode:
// floor2(x/y) -> (q,r). Liefere q und x-y*q=y*r.
	var cl_RA_div_t q_r = floor2(x/y);
	var cl_I& q = q_r.quotient;
