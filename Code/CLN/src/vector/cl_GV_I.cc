...
struct CLN_API cl_GV_I_vectorops {
...
struct CLN_API cl_heap_GV_I_general : public cl_heap_GV_I {
	cl_I data[1];
...
CLN_API cl_heap_GV_I* cl_make_heap_GV_I (std::size_t len)
{
...
static void bits_copy (const uintD* srcptr, std::size_t srcindex, uintD* destptr, std::size_t destindex, std::size_t count)
{
	...
	if (srcindex == destindex) {
		...
		if (count == 0)
			...
			lastdest = shiftleftcopy_loop_up(srcptr,destptr,static_cast<uintC>(count), static_cast<uintC>(i));
			*destptr |= tmp;
		}
...
CLN_API cl_heap_GV_I* cl_make_heap_GV_I (std::size_t len, sintC m)
{
	// Determine log2(bits).
	uintL log2_bits;
	switch (m) {
...
// An empty vector.
const CLN_API cl_GV_I cl_null_GV_I = cl_null_GV_I;
