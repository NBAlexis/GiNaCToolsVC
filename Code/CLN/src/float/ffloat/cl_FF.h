...
{
	return static_cast<uint32>((sign << FF_sign_shift)
	       | (exp << FF_exp_shift)
...
#else
// Single Float 0.0
  extern CLN_API const cl_FF cl_FF_0;
// Single Float 1.0
  extern CLN_API const cl_FF cl_FF_1;
// Single Float -1.0
  extern CLN_API const cl_FF cl_FF_minus1;
#endif
...
inline const cl_FF encode_FF (cl_signean sign, sintE exp, uintL mant)
{
	...
	return make_FF(sign, static_cast<unsigned int>(exp+FF_exp_mid), mant & (bit(FF_mant_len)-1));
}
...
extern CLN_API const cl_FF futruncate (const cl_FF& x);
...
extern CLN_API const cl_FF cl_I_to_FF (const cl_I& x);

// cl_RA_to_FF(x) wandelt eine rationale Zahl x in ein Single-Float um
// und rundet dabei.
extern CLN_API const cl_FF cl_RA_to_FF (const cl_RA& x);
...
extern CLN_API void cl_FF_to_float (const cl_FF& obj, ffloatjanus* val_);
