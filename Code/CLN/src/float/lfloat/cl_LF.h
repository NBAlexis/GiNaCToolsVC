...
extern CLN_API const cl_LF futruncate (const cl_LF& x);
...
extern CLN_API const cl_LF shorten (const cl_LF& x, uintC len);
...
extern CLN_API const cl_LF extend (const cl_LF& x, uintC len);
...
extern CLN_API const cl_LF LF_to_LF (const cl_LF& x, uintC len);
...
// LF_LF_plus_LF(x)
extern CLN_API const cl_LF LF_LF_plus_LF (const cl_LF& x, const cl_LF& y);

// Liefert zu zwei gleichlangen Long-Float x und y : (- x y), ein LF.
// LF_LF_minus_LF(x)
extern CLN_API const cl_LF LF_LF_minus_LF (const cl_LF& x, const cl_LF& y);
...
extern CLN_API const cl_LF cl_I_to_LF (const cl_I& x, uintC len);

// cl_RA_to_LF(x,len) wandelt eine rationale Zahl x in ein Long-Float um
// und rundet dabei.
extern CLN_API const cl_LF cl_RA_to_LF (const cl_RA& x, uintC len);

// cl_LF_I_mul(x,y) multipliziert ein Long-Float x und ein Integer y.
extern CLN_API const cl_R cl_LF_I_mul (const cl_LF& x, const cl_I& y);
...
extern CLN_API const cl_R cl_LF_RA_mul (const cl_LF& x, const cl_RA& y);
...
extern CLN_API uintC cl_LF_len_incsqrt (uintC len);

// Vergrößert eine Long-Float-Länge n, so daß aus d = intDsize*n
// mindestens d+sqrt(d)+2+(LF_exp_len-1) wird.
extern CLN_API uintC cl_LF_len_incsqrtx (uintC len);
...
extern CLN_API const cl_LF cl_LF_shortenrelative (const cl_LF& x, const cl_LF& y);

// cl_LF_shortenwith(x,y) tries to reduce the size of x, such that still
// 1 ulp(x) < y. y must be >0.
extern CLN_API const cl_LF cl_LF_shortenwith (const cl_LF& x, const cl_LF& y);
