...
not_rational_syntax:
	if (flags.syntax & syntax_float) {
		...
		if ((ptr != string_limit) && (*ptr == '_')) {
			...
			var cl_I prec1 = digits_to_I(ptr,static_cast<uintC>(ptr_after_prec-ptr),10);
			...
			if (flags.float_flags.mantissa_dependent_float_format) {
				...
				var uintC num_significant_digits = static_cast<uintC>(
		...
		return read_float(float_base,prec,sign,ptr_after_sign,0, static_cast<uintC>(ptr_after_fracpart-ptr_after_sign), static_cast<uintC>(ptr_after_exponent-ptr_after_sign), static_cast<uintC>(ptr_before_fracpart-ptr_after_sign));
	}
not_float_syntax:
