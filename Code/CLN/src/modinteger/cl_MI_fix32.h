...
{
	var uint32 xr = static_cast<uint32>(FN_to_UV(x.rep));
	var uint32 yr = static_cast<uint32>(FN_to_UV(y.rep));
	var uint32 zr = xr + yr;
	var uint32 m = static_cast<uint32>(FN_to_UV(R->modulus));
...
static const _cl_MI fix32_minus (cl_heap_modint_ring* R, const _cl_MI& x, const _cl_MI& y)
{
	var uint32 xr = static_cast<uint32>(FN_to_UV(x.rep));
	var uint32 yr = static_cast<uint32>(FN_to_UV(y.rep));
	var sint32 zr = static_cast<sint32>((xr >= yr ? xr - yr : xr - yr + FN_to_UV(R->modulus)));
	return _cl_MI(R, L_to_FN(zr));
}
...
{
	var uint32 xr = static_cast<uint32>(FN_to_UV(x.rep));
	var uint32 zr = static_cast<uint32>((xr==0 ? 0 : FN_to_UV(R->modulus)-xr));
	return _cl_MI(R, L_to_FN(zr));
}
