...
extern CLN_API const cl_LF operator- (const cl_LF& x);

// compare(x,y) vergleicht zwei Long-Floats x und y.
// Ergebnis: 0 falls x=y, +1 falls x>y, -1 falls x<y.
extern CLN_API cl_signean compare (const cl_LF& x, const cl_LF& y);

// equal_hashcode(x) liefert einen equal-invarianten Hashcode für x.
extern CLN_API uint32 equal_hashcode (const cl_LF& x);
...
// minusp(x) == (< x 0)
extern CLN_API bool minusp (const cl_LF& x);

// zerop(x) stellt fest, ob ein Long-Float x = 0.0 ist.
extern CLN_API bool zerop (const cl_LF& x);

// plusp(x) == (> x 0)
extern CLN_API bool plusp (const cl_LF& x);

// Liefert zu zwei Long-Float x und y : (+ x y), ein LF.
extern CLN_API const cl_LF operator+ (const cl_LF& x, const cl_LF& y);
...
extern CLN_API const cl_R cl_LF_I_mul(const cl_LF&, const cl_I&);
...
extern CLN_API const cl_R cl_LF_RA_mul(const cl_LF&, const cl_RA&);
...
extern CLN_API const cl_LF square (const cl_LF& x);

// Liefert zu zwei Long-Float x und y : (/ x y), ein LF.
extern CLN_API const cl_LF operator/ (const cl_LF& x, const cl_LF& y);
extern CLN_API const cl_LF cl_LF_I_div(const cl_LF& x, const cl_I& y);
// Spezialfall x oder y Integer oder rationale Zahl.
inline const cl_LF operator/ (const cl_LF& x, const cl_I& y)
{
	return cl_LF_I_div(x,y);
}
extern CLN_API const cl_R cl_I_LF_div(const cl_I& x, const cl_LF& y);
inline const cl_R operator/ (const cl_I& x, const cl_LF& y)
{
	return cl_I_LF_div(x,y);
}
extern CLN_API const cl_LF cl_LF_RA_div(const cl_LF& x, const cl_RA& y);
inline const cl_LF operator/ (const cl_LF& x, const cl_RA& y)
{
	return cl_LF_RA_div(x,y);
}
extern CLN_API const cl_R cl_RA_LF_div(const cl_RA& x, const cl_LF& y);
inline const cl_R operator/ (const cl_RA& x, const cl_LF& y)
{
...
extern CLN_API const cl_LF sqrt (const cl_LF& x);

// recip(x) liefert (/ x), wo x ein Long-Float ist.
extern CLN_API const cl_LF recip (const cl_LF& x);

// abs(x) liefert (abs x), wo x ein Long-Float ist.
extern CLN_API const cl_LF abs (const cl_LF& x);


// (1+ x), wo x ein Long-Float ist.
extern CLN_API const cl_LF plus1 (const cl_LF& x);

// (1- x), wo x ein Long-Float ist.
extern CLN_API const cl_LF minus1 (const cl_LF& x);


// ffloor(x) liefert (ffloor x), wo x ein LF ist.
extern CLN_API const cl_LF ffloor (const cl_LF& x);

// fceiling(x) liefert (fceiling x), wo x ein LF ist.
extern CLN_API const cl_LF fceiling (const cl_LF& x);

// ftruncate(x) liefert (ftruncate x), wo x ein LF ist.
extern CLN_API const cl_LF ftruncate (const cl_LF& x);

// fround(x) liefert (fround x), wo x ein LF ist.
extern CLN_API const cl_LF fround (const cl_LF& x);


// Return type for frounding operators.
// x / y  --> (q,r) with x = y*q+r.
struct CLN_API cl_LF_fdiv_t {
	cl_LF quotient;
	cl_LF remainder;
// Constructor.
	cl_LF_fdiv_t () {}
...
extern CLN_API const cl_LF LF_LF_minus_LF(const cl_LF&, const cl_LF&);
...
extern CLN_API const cl_LF LF_LF_minus_LF(const cl_LF&, const cl_LF&);
extern CLN_API const cl_I cl_LF_to_I(const cl_LF& x);
...
extern CLN_API const decoded_lfloat decode_float (const cl_LF& x);

// float_exponent(x) liefert zu einem Float x:
// den Exponenten von (decode-float x).
// x = 0.0 liefert 0.
// x = (-1)^s * 2^e * m liefert e.
extern CLN_API sintE float_exponent (const cl_LF& x);
...
extern CLN_API const cl_LF float_sign (const cl_LF& x);

// float_digits(x) liefert (float-digits x), wo x ein Float ist.
// < ergebnis: ein uintC >0
extern CLN_API uintC float_digits (const cl_LF& x);

// float_precision(x) liefert (float-precision x), wo x ein Float ist.
// < ergebnis: ein uintC >=0
extern CLN_API uintC float_precision (const cl_LF& x);
...
extern CLN_API const cl_idecoded_float integer_decode_float (const cl_LF& x);


// scale_float(x,delta) liefert x*2^delta, wo x ein LF ist.
extern CLN_API const cl_LF scale_float (const cl_LF& x, sintC delta);
extern CLN_API const cl_LF scale_float (const cl_LF& x, const cl_I& delta);


// max(x,y) liefert (max x y), wo x und y Floats sind.
extern CLN_API const cl_LF max (const cl_LF& x, const cl_LF& y);

// min(x,y) liefert (min x y), wo x und y Floats sind.
extern CLN_API const cl_LF min (const cl_LF& x, const cl_LF& y);

// signum(x) liefert (signum x), wo x ein Float ist.
extern CLN_API const cl_LF signum (const cl_LF& x);


// Konversion zu einem C "float".
extern CLN_API float float_approx (const cl_LF& x);

// Konversion zu einem C "double".
extern CLN_API double double_approx (const cl_LF& x);
...
extern CLN_API cl_class cl_class_lfloat;


// Debugging support.
#ifdef CL_DEBUG
extern CLN_API int cl_LF_debug_module;
