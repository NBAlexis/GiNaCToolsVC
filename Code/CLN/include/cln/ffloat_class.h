...
class CLN_API cl_FF : public cl_F {
public:
// Default constructor.
    inline cl_FF ();
// Assignment operators.
    inline cl_FF& operator= (const cl_FF&);
...
// Faster pointer_p() gives a faster copy constructor (but not destructor!!!).
    inline cl_FF (const cl_FF& x);
// Constructors and assignment operators from C numeric types.
	inline cl_FF (const float);
    inline cl_FF& operator= (const float);
// Other constructors.
	cl_FF (const char *);
// Private constructor.
    inline cl_FF (cl_private_thing);
#if defined(CL_WIDE_POINTERS)
