...
extern CLN_API const cl_I numerator (const cl_RA& r);

// denominator(r) liefert den Nenner (> 0) der rationalen Zahl r.
extern CLN_API const cl_I denominator (const cl_RA& r);


// Liefert (- r), wo r eine rationale Zahl ist.
extern CLN_API const cl_RA operator- (const cl_RA& r);

// (+ r s), wo r und s rationale Zahlen sind.
extern CLN_API const cl_RA operator+ (const cl_RA& r, const cl_RA& s);
...
extern CLN_API const cl_RA plus1 (const cl_RA& r);

// (1- r), wo r eine rationale Zahl ist.
extern CLN_API const cl_RA minus1 (const cl_RA& r);

// (abs r), wo r eine rationale Zahl ist.
extern CLN_API const cl_RA abs (const cl_RA& r);

// equal(r,s) vergleicht zwei rationale Zahlen r und s auf Gleichheit.
extern CLN_API bool equal (const cl_RA& r, const cl_RA& s);
// equal_hashcode(r) liefert einen equal-invarianten Hashcode für r.
extern CLN_API uint32 equal_hashcode (const cl_RA& r);

// compare(r,s) vergleicht zwei rationale Zahlen r und s.
// Ergebnis: 0 falls r=s, +1 falls r>s, -1 falls r<s.
extern CLN_API cl_signean compare (const cl_RA& r, const cl_RA& s);
...
// minusp(x) == (< x 0)
extern CLN_API bool minusp (const cl_RA& x);

// zerop(x) stellt fest, ob eine rationale Zahl = 0 ist.
extern CLN_API bool zerop (const cl_RA& x);

// plusp(x) == (> x 0)
extern CLN_API bool plusp (const cl_RA& x);

// Kehrwert (/ r), wo r eine rationale Zahl ist.
extern CLN_API const cl_RA recip (const cl_RA& r);
...
extern CLN_API const cl_RA square (const cl_RA& r);
...
// < q,r: Quotient q, ein Integer, Rest r, eine rationale Zahl
  extern CLN_API const cl_RA_div_t floor2 (const cl_RA& x);
  extern CLN_API const cl_I floor1 (const cl_RA& x);
...
// > x: rationale Zahl
// < q,r: Quotient q, ein Integer, Rest r, eine rationale Zahl
  extern CLN_API const cl_RA_div_t ceiling2 (const cl_RA& x);
  extern CLN_API const cl_I ceiling1 (const cl_RA& x);
...
// > x: rationale Zahl
// < q,r: Quotient q, ein Integer, Rest r, eine rationale Zahl
  extern CLN_API const cl_RA_div_t truncate2 (const cl_RA& x);
  extern CLN_API const cl_I truncate1 (const cl_RA& x);
...
// > x: rationale Zahl
// < q,r: Quotient q, ein Integer, Rest r, eine rationale Zahl
  extern CLN_API const cl_RA_div_t round2 (const cl_RA& x);
  extern CLN_API const cl_I round1 (const cl_RA& x);

// floor2(x,y) liefert (floor x y).
extern CLN_API const cl_RA_div_t floor2 (const cl_RA& x, const cl_RA& y);
extern CLN_API const cl_I floor1 (const cl_RA& x, const cl_RA& y);

// ceiling2(x,y) liefert (ceiling x y).
extern CLN_API const cl_RA_div_t ceiling2 (const cl_RA& x, const cl_RA& y);
extern CLN_API const cl_I ceiling1 (const cl_RA& x, const cl_RA& y);

// truncate2(x,y) liefert (truncate x y).
extern CLN_API const cl_RA_div_t truncate2 (const cl_RA& x, const cl_RA& y);
extern CLN_API const cl_I truncate1 (const cl_RA& x, const cl_RA& y);

// round2(x,y) liefert (round x y).
extern CLN_API const cl_RA_div_t round2 (const cl_RA& x, const cl_RA& y);
extern CLN_API const cl_I round1 (const cl_RA& x, const cl_RA& y);

// max(x,y) liefert (max x y), wo x und y rationale Zahlen sind.
extern CLN_API const cl_RA max (const cl_RA& x, const cl_RA& y);

// min(x,y) liefert (min x y), wo x und y rationale Zahlen sind.
extern CLN_API const cl_RA min (const cl_RA& x, const cl_RA& y);

// signum(x) liefert (signum x), wo x eine rationale Zahl ist.
extern CLN_API const cl_RA signum (const cl_RA& x);

// (expt x y), wo x eine rationale Zahl und y ein Integer >0 ist.
extern CLN_API const cl_RA expt_pos (const cl_RA& x, uintL y);
extern CLN_API const cl_RA expt_pos (const cl_RA& x, const cl_I& y);

// (expt x y), wo x eine rationale Zahl und y ein Integer ist.
extern CLN_API const cl_RA expt (const cl_RA& x, sintL y);
extern CLN_API const cl_RA expt (const cl_RA& x, const cl_I& y);
...
// < ergebnis: true      ..................., false sonst
  extern CLN_API bool sqrtp (const cl_RA& x, cl_RA* w);
...
// < ergebnis: true                       ........................, false sonst
  extern CLN_API bool rootp (const cl_RA& x, uintL n, cl_RA* w);
  extern CLN_API bool rootp (const cl_RA& x, const cl_I& n, cl_RA* w);
...
// < ergebnis: true    ......................................., false sonst
  extern CLN_API bool logp (const cl_I& a, const cl_I& b, cl_RA* l);
...
// < l: log(a,b)       falls er eine exakte rationale Zahl ist
// < ergebnis: true    ......................................., false sonst
  extern CLN_API bool logp (const cl_RA& a, const cl_RA& b, cl_RA* l);

// Konversion zu einem C "float".
extern CLN_API float float_approx (const cl_RA& x);

// Konversion zu einem C "double".
extern CLN_API double double_approx (const cl_RA& x);
...
extern CLN_API cl_class cl_class_ratio;


// Debugging support.
#ifdef CL_DEBUG
extern CLN_API int cl_RA_debug_module;
