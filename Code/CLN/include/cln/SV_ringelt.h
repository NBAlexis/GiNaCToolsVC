...
struct CLN_API cl_SV_ringelt : public cl_SV<_cl_ring_element,cl_SV_any> {
public:
	// Constructors.
	cl_SV_ringelt ();
...
extern CLN_API cl_heap_SV_ringelt* cl_make_heap_SV_ringelt_uninit (std::size_t len);
// Returns a new simple vector with all elements initialized to some value.
extern CLN_API cl_heap_SV_ringelt* cl_make_heap_SV_ringelt (std::size_t len);
...
class CLN_API cl_SV_ringelt_init_helper
{
	static int count;
public:
...
// Output.
extern CLN_API void fprint (std::ostream& stream, const cl_ring& R, const cl_SV_ringelt& x);
