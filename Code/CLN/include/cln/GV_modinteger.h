...
template <>
struct CLN_TEMPLATE cl_heap_GV<_cl_MI> : cl_heap {
	cl_GV_inner<_cl_MI> v;
	// here room for the elements
};
typedef cl_heap_GV<_cl_MI> cl_heap_GV_MI;

struct CLN_API cl_GV_MI : public cl_GV<_cl_MI,cl_GV_any> {
public:
	// Constructors.
    inline cl_GV_MI ();
    inline cl_GV_MI (const cl_GV_MI&);
	// Create a vector of modular integers.
    inline cl_GV_MI (std::size_t len, cl_heap_modint_ring* R);
	// Assignment operators.
    inline cl_GV_MI& operator= (const cl_GV_MI&);
