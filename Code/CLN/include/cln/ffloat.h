...
extern CLN_API const cl_FF operator- (const cl_FF& x);

// compare(x,y) vergleicht zwei Single-Floats x und y.
// Ergebnis: 0 falls x=y, +1 falls x>y, -1 falls x<y.
extern CLN_API cl_signean compare (const cl_FF& x, const cl_FF& y);

// equal_hashcode(x) liefert einen equal-invarianten Hashcode für x.
extern CLN_API uint32 equal_hashcode (const cl_FF& x);
...
// minusp(x) == (< x 0)
extern CLN_API bool minusp (const cl_FF& x);

// zerop(x) stellt fest, ob ein Single-Float x = 0.0 ist.
extern CLN_API bool zerop (const cl_FF& x);

// plusp(x) == (> x 0)
extern CLN_API bool plusp (const cl_FF& x);

// Liefert zu zwei Single-Float x und y : (+ x y), ein FF.
extern CLN_API const cl_FF operator+ (const cl_FF& x, const cl_FF& y);
...
extern CLN_API const cl_FF sqrt (const cl_FF& x);

// recip(x) liefert (/ x), wo x ein Single-Float ist.
extern CLN_API const cl_FF recip (const cl_FF& x);

// abs(x) liefert (abs x), wo x ein Single-Float ist.
extern CLN_API const cl_FF abs (const cl_FF& x);

extern CLN_API const cl_FF cl_I_to_FF(const cl_I&);
...
extern CLN_API const cl_FF ffloor (const cl_FF& x);

// fceiling(x) liefert (fceiling x), wo x ein FF ist.
extern CLN_API const cl_FF fceiling (const cl_FF& x);

// ftruncate(x) liefert (ftruncate x), wo x ein FF ist.
extern CLN_API const cl_FF ftruncate (const cl_FF& x);

// fround(x) liefert (fround x), wo x ein FF ist.
extern CLN_API const cl_FF fround (const cl_FF& x);

// Return type for frounding operators.
// x / y  --> (q,r) with x = y*q+r.
struct CLN_API cl_FF_fdiv_t {
	cl_FF quotient;
	cl_FF remainder;
// Constructor.
	cl_FF_fdiv_t () {}
...
extern CLN_API const cl_I cl_FF_to_I(const cl_FF& x);
...
extern CLN_API const decoded_ffloat decode_float (const cl_FF& x);

// float_exponent(x) liefert zu einem Float x:
// den Exponenten von (decode-float x).
// x = 0.0 liefert 0.
// x = (-1)^s * 2^e * m liefert e.
extern CLN_API sintE float_exponent (const cl_FF& x);
...
extern CLN_API const cl_FF float_sign (const cl_FF& x);

// float_digits(x) liefert (float-digits x), wo x ein Float ist.
// < ergebnis: ein uintC >0
extern CLN_API uintC float_digits (const cl_FF& x);

// float_precision(x) liefert (float-precision x), wo x ein Float ist.
// < ergebnis: ein uintC >=0
extern CLN_API uintC float_precision (const cl_FF& x);
...
extern CLN_API const cl_idecoded_float integer_decode_float (const cl_FF& x);


// scale_float(x,delta) liefert x*2^delta, wo x ein FF ist.
extern CLN_API const cl_FF scale_float (const cl_FF& x, sintC delta);
extern CLN_API const cl_FF scale_float (const cl_FF& x, const cl_I& delta);


// max(x,y) liefert (max x y), wo x und y Floats sind.
extern CLN_API const cl_FF max (const cl_FF& x, const cl_FF& y);

// min(x,y) liefert (min x y), wo x und y Floats sind.
extern CLN_API const cl_FF min (const cl_FF& x, const cl_FF& y);

// signum(x) liefert (signum x), wo x ein Float ist.
extern CLN_API const cl_FF signum (const cl_FF& x);


// Konversion zu einem C "float".
extern CLN_API float float_approx (const cl_FF& x);

// Konversion zu einem C "double".
extern CLN_API double double_approx (const cl_FF& x);
...
extern CLN_API cl_class cl_class_ffloat;
...
// Debugging support.
#ifdef CL_DEBUG
extern CLN_API int cl_FF_debug_module;
