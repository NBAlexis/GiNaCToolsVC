...
namespace cln {
#if (cl_value_len < 32)
extern CLN_API cl_private_thing cl_I_constructor_from_L (sint32 wert);
extern CLN_API cl_private_thing cl_I_constructor_from_UL (uint32 wert);
#endif
#ifdef intQsize
extern CLN_API cl_private_thing cl_I_constructor_from_Q (sint64 wert);
extern CLN_API cl_private_thing cl_I_constructor_from_UQ (uint64 wert);
#endif
}
#endif
...
#define CL_DEFINE_LONG_CONSTRUCTOR(_class_,_type_,_type2_)  \
inline _class_::_class_ (const _type_ wert)				\
{									\
	word = cl_combine(cl_FN_tag,(_type2_)wert);				\
}
#define CL_DEFINE_LONG_ASSIGNMENT_OPERATOR(_class_,_type_,_type2_)  \
inline _class_& _class_::operator= (const _type_ wert)			\
{									\
	cl_dec_refcount(*this);						\
	word = cl_combine(cl_FN_tag,(_type2_)wert);				\
...
// In MinGW64 although long bit size = 32, cl_value_len>32, the function cl_I_constructor_from_L and cl_I_constructor_from_UL is unimplemented
#define CL_DEFINE_LONG_CONSTRUCTORS(_class_)  \
CL_DEFINE_LONG_CONSTRUCTOR(_class_, long, int)					\
CL_DEFINE_LONG_CONSTRUCTOR(_class_, unsigned long, unsigned int)
...
CL_DEFINE_LONG_ASSIGNMENT_OPERATOR(_class_, long, int)				\
CL_DEFINE_LONG_ASSIGNMENT_OPERATOR(_class_, unsigned long, unsigned int)
...
// from `float':
extern CLN_API cl_private_thing cl_float_to_FF_pointer (const float val);
...
// from `double':
extern CLN_API struct cl_heap_dfloat * cl_double_to_DF_pointer (const double val);
...
class CLN_API cl_number : public cl_gcobject {
public:
...
// return value, and of course for objects with initializers of the same type.)
    inline cl_number (const cl_number& x);
// Converters. (Used for function argument passing and function return values.)
// Assignment operators. (Used for assignments.)
    inline cl_number& operator= (const cl_number&);
// Constructors and assignment operators from C numeric types.
    inline cl_number (const int);		// |argument| must be < 2^29
    inline cl_number (const unsigned int);	// argument must be < 2^29
    inline cl_number (const long);
    inline cl_number (const unsigned long);
#ifdef HAVE_LONGLONG
    inline cl_number (const long long);
    inline cl_number (const unsigned long long);
#endif
    inline cl_number (const float);
    inline cl_number (const double);
    inline cl_number& operator= (const int);	// |argument| must be < 2^29
    inline cl_number& operator= (const unsigned int); // argument must be < 2^29
    inline cl_number& operator= (const long);
    inline cl_number& operator= (const unsigned long);
    inline cl_number& operator= (const float);
    inline cl_number& operator= (const double);
#ifdef HAVE_LONGLONG
    inline cl_number& operator= (const long long);
    inline cl_number& operator= (const unsigned long long);
#endif
// Other constructors.
//	cl_number (const char *);
// Private pointer manipulations.
    inline cl_number (cln::cl_private_thing);
};

// Private constructors.
inline cl_number::cl_number (cln::cl_private_thing ptr) : cl_gcobject (ptr) {}
