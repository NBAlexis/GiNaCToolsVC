...
struct CLN_API cl_time_duration {
...
struct CLN_API cl_time_consumption {
	cl_time_duration realtime;	// elapsed time
	cl_time_duration usertime;	// system's notion of user time/run time
};

extern CLN_API const cl_time_duration operator- (const cl_timespec&, const cl_timespec&);
extern CLN_API const cl_timespec operator+ (const cl_timespec&, const cl_time_duration&);
extern CLN_API const cl_timespec operator- (const cl_timespec&, const cl_time_duration&);
extern CLN_API const cl_time_duration operator+ (const cl_time_duration&, const cl_time_duration&);
extern CLN_API const cl_time_duration operator- (const cl_time_duration&, const cl_time_duration&);

extern CLN_API const cl_timespec cl_current_time ();
extern CLN_API const cl_time_consumption cl_current_time_consumption ();

// Report a time consumption.
// (Should better be a virtual member function of `cl_time_consumption').
extern CLN_API void cl_timing_report (std::ostream&, const cl_time_consumption&);

struct CLN_API cl_timing {
