...
struct CLN_API cl_idecoded_float {
	cl_I mantissa;
	cl_I exponent;
	cl_I sign;
// Constructor.
...
// < ergebnis: (float x f)
extern CLN_API const cl_F cl_float (const cl_F& x, float_format_t f);
...
inline CLN_API const cl_F cl_float (const cl_F& x) { return x; }
...
// > y: ein Float
// < ergebnis: (float x y)
extern CLN_API const cl_F cl_float (const cl_I& x, const cl_F& y);
...
// > f: eine Float-Format-Spezifikation
// < ergebnis: (float x f)
extern CLN_API const cl_F cl_float (const cl_I& x, float_format_t f);

// cl_float(x) wandelt ein Integer x in ein Float um und rundet dabei.
// > x: ein Integer
// < ergebnis: (float x)
// Abhängig von default_float_format.
extern CLN_API const cl_F cl_float (const cl_I& x);
...
// > y: ein Float
// < ergebnis: (float x y)
extern CLN_API const cl_F cl_float (const cl_RA& x, const cl_F& y);
...
// > f: eine Float-Format-Spezifikation
// < ergebnis: (float x f)
extern CLN_API const cl_F cl_float (const cl_RA& x, float_format_t f);

// cl_float(x) wandelt eine rationale Zahl x in ein Float um und rundet dabei.
// > x: eine rationale Zahl
// < ergebnis: (float x)
// Abhängig von default_float_format.
extern CLN_API const cl_F cl_float (const cl_RA& x);
...
extern CLN_API const cl_F operator- (const cl_F& x);

// Liefert (+ x y), wo x und y Floats sind.
extern CLN_API const cl_F operator+ (const cl_F& x, const cl_F& y);
// The C++ compilers could hardly guess the following:
...
extern CLN_API const cl_R cl_F_I_mul(const cl_F&, const cl_I&);
...
extern CLN_API const cl_R cl_F_RA_mul(const cl_F&, const cl_RA&);
...
extern CLN_API const cl_F square (const cl_F& x);

// Liefert (/ x y), wo x und y Floats sind.
extern CLN_API const cl_F operator/ (const cl_F& x, const cl_F& y);
// Liefert (/ x y), wo x und y ein Float und eine rationale Zahl sind.
extern CLN_API const cl_F operator/ (const cl_F& x, const cl_RA& y);
extern CLN_API const cl_F operator/ (const cl_F& x, const cl_I& y);
extern CLN_API const cl_R operator/ (const cl_RA& x, const cl_F& y);
extern CLN_API const cl_R operator/ (const cl_I& x, const cl_F& y);
// The C++ compilers could hardly guess the following:
inline const cl_F operator/ (const cl_F& x, const int y)
	{ return x / cl_I(y); }
inline const cl_F operator/ (const cl_F& x, const unsigned int y)
...
extern CLN_API const cl_F abs (const cl_F& x);

// Liefert zu einem Float x>=0 : (sqrt x), ein Float.
extern CLN_API const cl_F sqrt (const cl_F& x);

// recip(x) liefert (/ x), wo x ein Float ist.
extern CLN_API const cl_F recip (const cl_F& x);
...
extern CLN_API cl_signean compare (const cl_F& x, const cl_F& y);

// equal_hashcode(x) liefert einen equal-invarianten Hashcode für x.
extern CLN_API uint32 equal_hashcode (const cl_F& x);
...
extern CLN_API const cl_F ffloor (const cl_F& x);

// fceiling(x) liefert (fceiling x), wo x ein Float ist.
extern CLN_API const cl_F fceiling (const cl_F& x);

// ftruncate(x) liefert (ftruncate x), wo x ein Float ist.
extern CLN_API const cl_F ftruncate (const cl_F& x);

// fround(x) liefert (fround x), wo x ein Float ist.
extern CLN_API const cl_F fround (const cl_F& x);


// Return type for frounding operators.
// x / y  --> (q,r) with x = y*q+r.
struct CLN_API cl_F_fdiv_t {
	cl_F quotient;
	cl_F remainder;
// Constructor.
	cl_F_fdiv_t () {}
...
extern CLN_API const cl_F_fdiv_t ffloor2 (const cl_F& x);

// fceiling2(x) liefert (fceiling x), wo x ein F ist.
extern CLN_API const cl_F_fdiv_t fceiling2 (const cl_F& x);

// ftruncate2(x) liefert (ftruncate x), wo x ein F ist.
extern CLN_API const cl_F_fdiv_t ftruncate2 (const cl_F& x);

// fround2(x) liefert (fround x), wo x ein F ist.
extern CLN_API const cl_F_fdiv_t fround2 (const cl_F& x);


// Return type for rounding operators.
// x / y  --> (q,r) with x = y*q+r.
struct CLN_API cl_F_div_t {
	cl_I quotient;
	cl_F remainder;
// Constructor.
	cl_F_div_t () {}
...
extern CLN_API const cl_F_div_t floor2 (const cl_F& x);
extern CLN_API const cl_I floor1 (const cl_F& x);

// ceiling2(x) liefert (ceiling x), wo x ein F ist.
extern CLN_API const cl_F_div_t ceiling2 (const cl_F& x);
extern CLN_API const cl_I ceiling1 (const cl_F& x);

// truncate2(x) liefert (truncate x), wo x ein F ist.
extern CLN_API const cl_F_div_t truncate2 (const cl_F& x);
extern CLN_API const cl_I truncate1 (const cl_F& x);

// round2(x) liefert (round x), wo x ein F ist.
extern CLN_API const cl_F_div_t round2 (const cl_F& x);
extern CLN_API const cl_I round1 (const cl_F& x);

// floor2(x,y) liefert (floor x y), wo x und y Floats sind.
extern CLN_API const cl_F_div_t floor2 (const cl_F& x, const cl_F& y);
...
extern CLN_API const cl_F_div_t ceiling2 (const cl_F& x, const cl_F& y);
...
extern CLN_API const cl_F_div_t truncate2 (const cl_F& x, const cl_F& y);
...
extern CLN_API const decoded_float decode_float (const cl_F& x);

// float_exponent(x) liefert zu einem Float x:
// den Exponenten von (decode-float x).
// x = 0.0 liefert 0.
// x = (-1)^s * 2^e * m liefert e.
extern CLN_API sintE float_exponent (const cl_F& x);
...
extern CLN_API const cl_F float_sign (const cl_F& x);

// float_sign(x,y) liefert (float-sign x y), wo x und y Floats sind.
extern CLN_API const cl_F float_sign (const cl_F& x, const cl_F& y);

// float_digits(x) liefert (float-digits x), wo x ein Float ist.
// < ergebnis: ein uintC >0
extern CLN_API uintC float_digits (const cl_F& x);

// float_precision(x) liefert (float-precision x), wo x ein Float ist.
// < ergebnis: ein uintC >=0
extern CLN_API uintC float_precision (const cl_F& x);
...
extern CLN_API const cl_idecoded_float integer_decode_float (const cl_F& x);


// rational(x) liefert (rational x), wo x ein Float ist.
extern CLN_API const cl_RA rational (const cl_F& x);


// scale_float(x,delta) liefert x*2^delta, wo x ein Float ist.
extern CLN_API const cl_F scale_float (const cl_F& x, sintC delta);
extern CLN_API const cl_F scale_float (const cl_F& x, const cl_I& delta);


// max(x,y) liefert (max x y), wo x und y Floats sind.
extern CLN_API const cl_F max (const cl_F& x, const cl_F& y);

// min(x,y) liefert (min x y), wo x und y Floats sind.
extern CLN_API const cl_F min (const cl_F& x, const cl_F& y);

// signum(x) liefert (signum x), wo x ein Float ist.
extern CLN_API const cl_F signum (const cl_F& x);


// Returns the largest (most positive) floating point number in float format f.
extern CLN_API const cl_F most_positive_float (float_format_t f);

// Returns the smallest (most negative) floating point number in float format f.
extern CLN_API const cl_F most_negative_float (float_format_t f);

// Returns the least positive floating point number (i.e. > 0 but closest to 0)
// in float format f.
extern CLN_API const cl_F least_positive_float (float_format_t f);

// Returns the least negative floating point number (i.e. < 0 but closest to 0)
// in float format f.
extern CLN_API const cl_F least_negative_float (float_format_t f);

// Returns the smallest floating point number e > 0 such that 1+e != 1.
extern CLN_API const cl_F float_epsilon (float_format_t f);

// Returns the smallest floating point number e > 0 such that 1-e != 1.
extern CLN_API const cl_F float_negative_epsilon (float_format_t f);


// Konversion zu einem C "float".
extern CLN_API float float_approx (const cl_F& x);

// Konversion zu einem C "double".
extern CLN_API double double_approx (const cl_F& x);
...
// > y: ein Float
extern CLN_API const cl_F pi (const cl_F& y);

// pi(y) liefert die Zahl pi im Float-Format f.
// > f: eine Float-Format-Spezifikation
extern CLN_API const cl_F pi (float_format_t f);

// pi() liefert die Zahl pi im Default-Float-Format.
extern CLN_API const cl_F pi (void);


// sin(x) liefert den Sinus (sin x) eines Float x.
extern CLN_API const cl_F sin (const cl_F& x);

// cos(x) liefert den Cosinus (cos x) eines Float x.
extern CLN_API const cl_F cos (const cl_F& x);

// Return type for cos_sin():
struct CLN_API cos_sin_t {
	cl_R cos;
	cl_R sin;
// Constructor:
	cos_sin_t () {}
...
extern CLN_API const cos_sin_t cos_sin (const cl_F& x);

// tan(x) liefert den Tangens (tan x) eines Float x.
extern CLN_API const cl_F tan (const cl_F& x);


// exp1(y) liefert die Zahl e = exp(1) im selben Float-Format wie y.
// > y: ein Float
extern CLN_API const cl_F exp1 (const cl_F& y);

// exp1(y) liefert die Zahl e = exp(1) im Float-Format f.
// > f: eine Float-Format-Spezifikation
extern CLN_API const cl_F exp1 (float_format_t f);

// exp1() liefert die Zahl e = exp(1) im Default-Float-Format.
extern CLN_API const cl_F exp1 (void);


// ln(x) liefert zu einem Float x>0 die Zahl ln(x).
extern CLN_API const cl_F ln (const cl_F& x);
...
extern CLN_API const cl_F exp (const cl_F& x);

// sinh(x) liefert zu einem Float x die Zahl sinh(x).
extern CLN_API const cl_F sinh (const cl_F& x);

// cosh(x) liefert zu einem Float x die Zahl cosh(x).
extern CLN_API const cl_F cosh (const cl_F& x);

// Return type for cosh_sinh():
struct CLN_API cosh_sinh_t {
	cl_R cosh;
	cl_R sinh;
// Constructor:
	cosh_sinh_t () {}
...
extern CLN_API const cosh_sinh_t cosh_sinh (const cl_F& x);

// tanh(x) liefert zu einem Float x die Zahl tanh(x).
extern CLN_API const cl_F tanh (const cl_F& x);


// eulerconst(y) liefert die Eulersche Konstante
// im selben Float-Format wie y.
// > y: ein Float
extern CLN_API const cl_F eulerconst (const cl_F& y);

// eulerconst(y) liefert die Eulersche Konstante im Float-Format f.
// > f: eine Float-Format-Spezifikation
extern CLN_API const cl_F eulerconst (float_format_t f);

// eulerconst() liefert die Eulersche Konstante im Default-Float-Format.
extern CLN_API const cl_F eulerconst (void);

// catalanconst(y) liefert die Catalansche Konstante
// im selben Float-Format wie y.
// > y: ein Float
extern CLN_API const cl_F catalanconst (const cl_F& y);

// catalanconst(y) liefert die Catalansche Konstante im Float-Format f.
// > f: eine Float-Format-Spezifikation
extern CLN_API const cl_F catalanconst (float_format_t f);

// catalanconst() liefert die Catalansche Konstante im Default-Float-Format.
extern CLN_API const cl_F catalanconst (void);

// zeta(s) returns the Riemann zeta function at s>1.
extern CLN_API const cl_F zeta (int s, const cl_F& y);
extern CLN_API const cl_F zeta (int s, float_format_t f);
extern CLN_API const cl_F zeta (int s);


// random_F(randomstate,n) liefert zu einem Float n>0 ein zufälliges
// Float x mit 0 <= x < n.
// > randomstate: ein Random-State, wird verändert
extern CLN_API const cl_F random_F (random_state& randomstate, const cl_F& n);
...
extern CLN_API bool cl_inhibit_floating_point_underflow;
