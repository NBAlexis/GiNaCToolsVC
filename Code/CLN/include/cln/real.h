...
extern CLN_API const cl_LF cl_R_to_LF (const cl_R& x, uintC len);
...
// > y: ein Float
// < ergebnis: (float x y)
extern CLN_API const cl_F cl_float (const cl_R& x, const cl_F& y);
...
// < ergebnis: (float x f)
extern CLN_API const cl_F cl_float (const cl_R& x, float_format_t f);
...
extern CLN_API const cl_F cl_float (const cl_R& x);


// Liefert (- x), wo x eine reelle Zahl ist.
extern CLN_API const cl_R operator- (const cl_R& x);

// Liefert (+ x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R operator+ (const cl_R& x, const cl_R& y);
...
extern CLN_API const cl_R square (const cl_R& x);
...
extern CLN_API const cl_R abs (const cl_R& x);

// recip(x) liefert (/ x), wo x eine reelle Zahl ist.
extern CLN_API const cl_R recip (const cl_R& x);

// (1+ x), wo x eine reelle Zahl ist.
extern CLN_API const cl_R plus1 (const cl_R& x);

// (1- x), wo x eine reelle Zahl ist.
extern CLN_API const cl_R minus1 (const cl_R& x);


// Return type for rounding operators.
// x / y  --> (q,r) with x = y*q+r.
struct CLN_API cl_R_div_t {
	cl_I quotient;
	cl_R remainder;
// Constructor.
	cl_R_div_t () {}
...
extern CLN_API const cl_R_div_t floor2 (const cl_R& x);
extern CLN_API const cl_I floor1 (const cl_R& x);

// ceiling2(x) liefert (ceiling x), wo x eine reelle Zahl ist.
extern CLN_API const cl_R_div_t ceiling2 (const cl_R& x);
extern CLN_API const cl_I ceiling1 (const cl_R& x);

// truncate2(x) liefert (truncate x), wo x eine reelle Zahl ist.
extern CLN_API const cl_R_div_t truncate2 (const cl_R& x);
extern CLN_API const cl_I truncate1 (const cl_R& x);

// round2(x) liefert (round x), wo x eine reelle Zahl ist.
extern CLN_API const cl_R_div_t round2 (const cl_R& x);
extern CLN_API const cl_I round1 (const cl_R& x);

// floor2(x,y) liefert (floor x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R_div_t floor2 (const cl_R& x, const cl_R& y);
extern CLN_API const cl_I floor1 (const cl_R& x, const cl_R& y);

// ceiling2(x,y) liefert (ceiling x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R_div_t ceiling2 (const cl_R& x, const cl_R& y);
extern CLN_API const cl_I ceiling1 (const cl_R& x, const cl_R& y);

// truncate2(x,y) liefert (truncate x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R_div_t truncate2 (const cl_R& x, const cl_R& y);
extern CLN_API const cl_I truncate1 (const cl_R& x, const cl_R& y);

// round2(x,y) liefert (round x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R_div_t round2 (const cl_R& x, const cl_R& y);
extern CLN_API const cl_I round1 (const cl_R& x, const cl_R& y);


// Return type for frounding operators.
// x / y  --> (q,r) with x = y*q+r.
struct CLN_API cl_R_fdiv_t {
	cl_F quotient;
	cl_R remainder;
// Constructor.
	cl_R_fdiv_t () {}
...
extern CLN_API const cl_R_fdiv_t ffloor2 (const cl_R& x);
extern CLN_API const cl_F ffloor (const cl_R& x);

// fceiling2(x) liefert (fceiling x), wo x eine reelle Zahl ist.
extern CLN_API const cl_R_fdiv_t fceiling2 (const cl_R& x);
extern CLN_API const cl_F fceiling (const cl_R& x);

// ftruncate2(x) liefert (ftruncate x), wo x eine reelle Zahl ist.
extern CLN_API const cl_R_fdiv_t ftruncate2 (const cl_R& x);
extern CLN_API const cl_F ftruncate (const cl_R& x);

// fround2(x) liefert (fround x), wo x eine reelle Zahl ist.
extern CLN_API const cl_R_fdiv_t fround2 (const cl_R& x);
extern CLN_API const cl_F fround (const cl_R& x);

// ffloor2(x,y) liefert (ffloor x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R_fdiv_t ffloor2 (const cl_R& x, const cl_R& y);
extern CLN_API const cl_F ffloor (const cl_R& x, const cl_R& y);

// fceiling2(x,y) liefert (fceiling x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R_fdiv_t fceiling2 (const cl_R& x, const cl_R& y);
extern CLN_API const cl_F fceiling (const cl_R& x, const cl_R& y);

// ftruncate2(x,y) liefert (ftruncate x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R_fdiv_t ftruncate2 (const cl_R& x, const cl_R& y);
extern CLN_API const cl_F ftruncate (const cl_R& x, const cl_R& y);

// fround2(x,y) liefert (fround x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R_fdiv_t fround2 (const cl_R& x, const cl_R& y);
extern CLN_API const cl_F fround (const cl_R& x, const cl_R& y);


// mod(x,y) = (mod x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R mod (const cl_R& x, const cl_R& y);

// rem(x,y) = (rem x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R rem (const cl_R& x, const cl_R& y);


// rational(x) liefert (rational x), wo x eine reelle Zahl ist.
extern CLN_API const cl_RA rational (const cl_R& x);
// Spezialfall:
inline const cl_RA rational (const cl_RA& x) { return x; }


// equal(x,y) vergleicht zwei reelle Zahlen x und y auf Gleichheit.
extern CLN_API bool equal (const cl_R& x, const cl_R& y);
// equal_hashcode(x) liefert einen equal-invarianten Hashcode für x.
extern CLN_API uint32 equal_hashcode (const cl_R& x);

// compare(x,y) vergleicht zwei reelle Zahlen x und y.
// Ergebnis: 0 falls x=y, +1 falls x>y, -1 falls x<y.
extern CLN_API cl_signean compare (const cl_R& x, const cl_R& y);
...
extern CLN_API const cl_R max (const cl_R& x, const cl_R& y);

// min(x,y) liefert (min x y), wo x und y reelle Zahlen sind.
extern CLN_API const cl_R min (const cl_R& x, const cl_R& y);

// signum(x) liefert (signum x), wo x eine reelle Zahl ist.
extern CLN_API const cl_R signum (const cl_R& x);

// sqrt(x) = (sqrt x) zieht die Wurzel aus einer reellen Zahl x >=0.
extern CLN_API const cl_R sqrt (const cl_R& x);
// sqrt(x) = (sqrt x) zieht die Wurzel aus einer rationalen Zahl x >=0.
extern CLN_API const cl_R sqrt (const cl_RA& x);

// (expt x y), wo x eine reelle Zahl und y ein Integer ist.
extern CLN_API const cl_R expt (const cl_R& x, sintL y);
extern CLN_API const cl_R expt (const cl_R& x, const cl_I& y);

// rationalize(x) liefert (rationalize x), wo x eine reelle Zahl ist.
extern CLN_API const cl_RA rationalize (const cl_R& x);


// Konversion zu einem C "float".
extern CLN_API float float_approx (const cl_R& x);

// Konversion zu einem C "double".
extern CLN_API double double_approx (const cl_R& x);
...
extern CLN_API const cl_R atan (const cl_R& x, const cl_R& y);
...
extern CLN_API const cl_R atan (const cl_R& x);
// Spezialfall: x Float -> Ergebnis Float
...
extern CLN_API const cl_R sin (const cl_R& x);
// Dem C++-Compiler muß man nun auch das Folgende sagen:
...
extern CLN_API const cl_R cos (const cl_R& x);
// Dem C++-Compiler muß man nun auch das Folgende sagen:
...
extern CLN_API const cos_sin_t cos_sin (const cl_R& x);

// tan(x) liefert den Tangens (tan x) einer reellen Zahl x.
extern CLN_API const cl_R tan (const cl_R& x);
// Dem C++-Compiler muß man nun auch das Folgende sagen:
...
extern CLN_API const cl_R ln (const cl_R& x);
// Dem C++-Compiler muß man nun auch das Folgende sagen:
...
extern CLN_API const cl_R log (const cl_R& a, const cl_R& b);

// exp(x) liefert zu einer reellen Zahl x die Zahl exp(x).
extern CLN_API const cl_R exp (const cl_R& x);
// Dem C++-Compiler muß man nun auch das Folgende sagen:
...
extern CLN_API const cl_R sinh (const cl_R& x);
// Dem C++-Compiler muß man nun auch das Folgende sagen:
...
extern CLN_API const cl_R cosh (const cl_R& x);
// Dem C++-Compiler muß man nun auch das Folgende sagen:
...
extern CLN_API const cosh_sinh_t cosh_sinh (const cl_R& x);

// tanh(x) liefert zu einer reellen Zahl x die Zahl tanh(x).
extern CLN_API const cl_R tanh (const cl_R& x);
// Dem C++-Compiler muß man nun auch das Folgende sagen:
...
// x mit 0 <= x < n.
extern CLN_API const cl_R random_R (random_state& randomstate, const cl_R& n);
...
// Debugging support.
#ifdef CL_DEBUG
extern CLN_API int cl_R_debug_module;
