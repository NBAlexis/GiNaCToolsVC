...
extern CLN_API const cl_DF operator- (const cl_DF& x);

// compare(x,y) vergleicht zwei Double-Floats x und y.
// Ergebnis: 0 falls x=y, +1 falls x>y, -1 falls x<y.
extern CLN_API cl_signean compare (const cl_DF& x, const cl_DF& y);

// equal_hashcode(x) liefert einen equal-invarianten Hashcode für x.
extern CLN_API uint32 equal_hashcode (const cl_DF& x);
...
// minusp(x) == (< x 0)
extern CLN_API bool minusp (const cl_DF& x);

// zerop(x) stellt fest, ob ein Double-Float x = 0.0 ist.
extern CLN_API bool zerop (const cl_DF& x);

// plusp(x) == (> x 0)
extern CLN_API bool plusp (const cl_DF& x);

// Liefert zu zwei Double-Float x und y : (+ x y), ein DF.
extern CLN_API const cl_DF operator+ (const cl_DF& x, const cl_DF& y);
...
extern CLN_API const cl_DF sqrt (const cl_DF& x);

// recip(x) liefert (/ x), wo x ein Double-Float ist.
extern CLN_API const cl_DF recip (const cl_DF& x);

// abs(x) liefert (abs x), wo x ein Double-Float ist.
extern CLN_API const cl_DF abs (const cl_DF& x);

extern CLN_API const cl_DF cl_I_to_DF(const cl_I&);
...
extern CLN_API const cl_DF ffloor (const cl_DF& x);

// fceiling(x) liefert (fceiling x), wo x ein DF ist.
extern CLN_API const cl_DF fceiling (const cl_DF& x);

// ftruncate(x) liefert (ftruncate x), wo x ein DF ist.
extern CLN_API const cl_DF ftruncate (const cl_DF& x);

// fround(x) liefert (fround x), wo x ein DF ist.
extern CLN_API const cl_DF fround (const cl_DF& x);


// Return type for frounding operators.
// x / y  --> (q,r) with x = y*q+r.
struct CLN_API cl_DF_fdiv_t {
	cl_DF quotient;
	cl_DF remainder;
// Constructor.
	cl_DF_fdiv_t () {}
...
// x / y  --> (q,r) with x = y*q+r.
struct CLN_API cl_DF_div_t {
	cl_I quotient;
	cl_DF remainder;
// Constructor.
    inline cl_DF_div_t () {}
	inline cl_DF_div_t (const cl_I& q, const cl_DF& r) : quotient(q), remainder(r) {}
};

extern CLN_API const cl_I cl_DF_to_I(const cl_DF& x);
...
extern CLN_API const decoded_dfloat decode_float (const cl_DF& x);

// float_exponent(x) liefert zu einem Float x:
// den Exponenten von (decode-float x).
// x = 0.0 liefert 0.
// x = (-1)^s * 2^e * m liefert e.
extern CLN_API sintE float_exponent (const cl_DF& x);
...
extern CLN_API const cl_DF float_sign (const cl_DF& x);

// float_digits(x) liefert (float-digits x), wo x ein Float ist.
// < ergebnis: ein uintC >0
extern CLN_API uintC float_digits (const cl_DF& x);

// float_precision(x) liefert (float-precision x), wo x ein Float ist.
// < ergebnis: ein uintC >=0
extern CLN_API uintC float_precision (const cl_DF& x);
...
extern CLN_API const cl_idecoded_float integer_decode_float (const cl_DF& x);


// scale_float(x,delta) liefert x*2^delta, wo x ein DF ist.
extern CLN_API const cl_DF scale_float (const cl_DF& x, sintC delta);
extern CLN_API const cl_DF scale_float (const cl_DF& x, const cl_I& delta);


// max(x,y) liefert (max x y), wo x und y Floats sind.
extern CLN_API const cl_DF max (const cl_DF& x, const cl_DF& y);

// min(x,y) liefert (min x y), wo x und y Floats sind.
extern CLN_API const cl_DF min (const cl_DF& x, const cl_DF& y);

// signum(x) liefert (signum x), wo x ein Float ist.
extern CLN_API const cl_DF signum (const cl_DF& x);


// Konversion zu einem C "float".
extern CLN_API float float_approx (const cl_DF& x);

// Konversion zu einem C "double".
extern CLN_API double double_approx (const cl_DF& x);
...
/* test futruncate */
extern CLN_API const cl_DF futruncate(const cl_DF& x);

// Runtime typing support.
extern CLN_API cl_class cl_class_dfloat;


// Debugging support.
#ifdef CL_DEBUG
extern CLN_API int cl_DF_debug_module;
