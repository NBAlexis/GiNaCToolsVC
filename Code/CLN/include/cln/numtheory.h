...
// a, b must be integers, b > 0, b odd. The result is 0 iff gcd(a,b) > 1.
  extern CLN_API int jacobi (sintV a, sintV b);
  extern CLN_API int jacobi (const cl_I& a, const cl_I& b);

// isprobprime(n), n integer > 0,
// returns true when n is probably prime.
// This is pretty quick, but no caching is done.
  extern CLN_API bool isprobprime (const cl_I& n);

// nextprobprime(x) returns the smallest probable prime >= x.
  extern CLN_API const cl_I nextprobprime (const cl_R& x);
...
extern CLN_API const sqrt_mod_p_t sqrt_mod_p (const cl_modint_ring& R, const cl_MI& x);
...
extern CLN_API const cornacchia_t cornacchia1 (const cl_I& d, const cl_I& p);
extern CLN_API const cornacchia_t cornacchia4 (const cl_I& d, const cl_I& p);
