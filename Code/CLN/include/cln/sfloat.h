...
extern CLN_API const cl_SF operator- (const cl_SF& x);

// compare(x,y) vergleicht zwei Short-Floats x und y.
// Ergebnis: 0 falls x=y, +1 falls x>y, -1 falls x<y.
extern CLN_API cl_signean compare (const cl_SF& x, const cl_SF& y);

// equal_hashcode(x) liefert einen equal-invarianten Hashcode für x.
extern CLN_API uint32 equal_hashcode (const cl_SF& x);
...
// minusp(x) == (< x 0)
extern CLN_API bool minusp (const cl_SF& x);

// zerop(x) stellt fest, ob ein Short-Float x = 0.0 ist.
extern CLN_API bool zerop (const cl_SF& x);

// plusp(x) == (> x 0)
extern CLN_API bool plusp (const cl_SF& x);

// Liefert zu zwei Short-Float x und y : (+ x y), ein SF.
extern CLN_API const cl_SF operator+ (const cl_SF& x, const cl_SF& y);
...
extern CLN_API const cl_SF sqrt (const cl_SF& x);

// recip(x) liefert (/ x), wo x ein Short-Float ist.
extern CLN_API const cl_SF recip (const cl_SF& x);

// abs(x) liefert (abs x), wo x ein Short-Float ist.
extern CLN_API const cl_SF abs (const cl_SF& x);


// (1+ x), wo x ein Short-Float ist.
extern CLN_API const cl_SF cl_I_to_SF(const cl_I&);
...
extern CLN_API const cl_SF ffloor (const cl_SF& x);

// fceiling(x) liefert (fceiling x), wo x ein SF ist.
extern CLN_API const cl_SF fceiling (const cl_SF& x);

// ftruncate(x) liefert (ftruncate x), wo x ein SF ist.
extern CLN_API const cl_SF ftruncate (const cl_SF& x);

// fround(x) liefert (fround x), wo x ein SF ist.
extern CLN_API const cl_SF fround (const cl_SF& x);


// Return type for frounding operators.
// x / y  --> (q,r) with x = y*q+r.
struct CLN_API cl_SF_fdiv_t {
	cl_SF quotient;
	cl_SF remainder;
// Constructor.
	cl_SF_fdiv_t () {}
...
{
	extern CLN_API const cl_I cl_SF_to_I (const cl_SF& x);
	cl_SF q = ffloor(x);
...
extern CLN_API const decoded_sfloat decode_float (const cl_SF& x);

// float_exponent(x) liefert zu einem Float x:
// den Exponenten von (decode-float x).
// x = 0.0 liefert 0.
// x = (-1)^s * 2^e * m liefert e.
extern CLN_API sintE float_exponent (const cl_SF& x);
...
extern CLN_API const cl_SF float_sign (const cl_SF& x);

// float_digits(x) liefert (float-digits x), wo x ein Float ist.
// < ergebnis: ein uintC >0
extern CLN_API uintC float_digits (const cl_SF& x);

// float_precision(x) liefert (float-precision x), wo x ein Float ist.
// < ergebnis: ein uintC >=0
extern CLN_API uintC float_precision (const cl_SF& x);
...
extern CLN_API const cl_idecoded_float integer_decode_float (const cl_SF& x);


// scale_float(x,delta) liefert x*2^delta, wo x ein SF ist.
extern CLN_API const cl_SF scale_float (const cl_SF& x, sintC delta);
extern CLN_API const cl_SF scale_float (const cl_SF& x, const cl_I& delta);


// max(x,y) liefert (max x y), wo x und y Floats sind.
extern CLN_API const cl_SF max (const cl_SF& x, const cl_SF& y);

// min(x,y) liefert (min x y), wo x und y Floats sind.
extern CLN_API const cl_SF min (const cl_SF& x, const cl_SF& y);

// signum(x) liefert (signum x), wo x ein Float ist.
extern CLN_API const cl_SF signum (const cl_SF& x);


// Konversion zu einem C "float".
extern CLN_API float float_approx (const cl_SF& x);

// Konversion zu einem C "double".
extern CLN_API double double_approx (const cl_SF& x);
...
extern CLN_API cl_class cl_class_sfloat;
CL_FORCE_LINK(cl_SF_classes_dummy, cl_class_sfloat)


// Debugging support.
#ifdef CL_DEBUG
extern CLN_API int cl_SF_debug_module;
