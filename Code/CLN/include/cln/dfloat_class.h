...
class CLN_API cl_DF : public cl_F {
public:
// Default constructor.
	cl_DF ();
...
// Faster pointer_p() gives a faster copy constructor (but not destructor!!!).
    inline cl_DF (const cl_DF& x);
// Constructors and assignment operators from C numeric types.
    inline cl_DF (const double);
    inline cl_DF& operator= (const double);
// Other constructors.
	cl_DF (const char *);
// Private constructor.
    inline cl_DF (cl_private_thing);
    inline cl_DF (struct cl_heap_dfloat *);
// Private pointer manipulations.
    inline operator struct cl_heap_dfloat * () const;
...
class CLN_API cl_DF_globals_init_helper
{
	static int count;
public:
