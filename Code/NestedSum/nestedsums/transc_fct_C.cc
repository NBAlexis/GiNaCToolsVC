...
/** @file transc_fct_C.cc
 *
 * Implementation of transcendental_fct_type_C
 *
 */
...
GINAC_IMPLEMENT_REGISTERED_CLASS(transcendental_fct_type_C, basic)

GINAC_BIND_UNARCHIVER(transcendental_fct_type_C);


  transcendental_fct_type_C::transcendental_fct_type_C() : inherited(),
  ...
  void transcendental_fct_type_C::archive(archive_node &node) const
  {
  ...
  void transcendental_fct_type_C::read_archive(const archive_node &node, lst &sym_lst)
  {
  ...
  int transcendental_fct_type_C::compare_same_type(const basic &other) const
  {
    const transcendental_fct_type_C &o = static_cast<const transcendental_fct_type_C &>(other);
  ...
  // other ctors
  transcendental_fct_type_C::transcendental_fct_type_C(const ex & xx1, const ex & xx2, const ex & ii_num, const ex & ii_denom, 
  ...
  unsigned transcendental_fct_type_C::calchash(void) const
    {
  ...
  void transcendental_fct_type_C::print(const print_context & c, unsigned level) const
  {
   ...
   *
   */
  ex transcendental_fct_type_C::eval() const
  {
#ifdef _DEBUG
    time_t time_start;
    time_t time_end;
	...
	time(&time_end);
	std::cout << "transcendental_fct_type_C, part 1, time in seconds : " << difftime(time_end,time_start) << std::endl;
#endif // def _DEBUG
	...
	// measuring time end here
	time(&time_end);
	std::cout << "transcendental_fct_type_C, part 2, time in seconds : " << difftime(time_end,time_start) << std::endl;
#endif // def _DEBUG
	...
	// measuring time end here
	time(&time_end);
	std::cout << "transcendental_fct_type_C, part 3, time in seconds : " << difftime(time_end,time_start) << std::endl;
#endif // def _DEBUG
	...
	// gamma sequences
	lst t_11_gammas_n, t_11_gammas_i;
	int flag_n = 0;
	for ( k=0;k<lst_ij_num.nops();k++)
	  {
	    temp_num = nestedsums_helper_expand(lst_ij_num.op(k));
	...
	// poles for n=1,... ?
	if (flag_n) return convert_Zsums_to_standard_form(term_00 + term_01 + this->shift_plus_one(), lst{expansion_parameter});
	...
	sum_n = create_transcendental_sum_type_C_and_expand(index_n,index,
							    create_letter(-x1*pow(x2,-1),0,index),
...
#ifdef _DEBUG
	time(&time_end);
	std::cout << "transcendental_fct_type_C, part 4, time in seconds : " << difftime(time_end,time_start) << std::endl;
	time(&time_start);
#endif // def _DEBUG
	...
	// measuring time end here
	time(&time_end);
	std::cout << "transcendental_fct_type_C, part 5, time in seconds : " << difftime(time_end,time_start) << std::endl;
	time(&time_start);
#endif // def _DEBUG
	...
	ex res = convert_Zsums_to_standard_form(term_00 + term_01 + term_11, lst{expansion_parameter});

#ifdef _DEBUG
	// measuring time end here
	time(&time_end);
	std::cout << "transcendental_fct_type_C, part 6, time in seconds : " << difftime(time_end,time_start) << std::endl;
#endif // def _DEBUG
   ...
   *
   */
  ex transcendental_fct_type_C::eval_ncmul(const exvector & v) const
  {
  ...
  ex transcendental_fct_type_C::derivative(const symbol & s) const
  {
    return 0;
  }

  ex transcendental_fct_type_C::subs(const exmap & m, unsigned options) const
  {
    exmap::const_iterator it = m.find(*this);
    if (it != m.end())
        return it->second;

    return create_transcendental_fct_type_C(x1.subs(m,options),x2.subs(m,options),
   ...
   *
   */
  ex transcendental_fct_type_C::set_expansion(const ex & eps, int o) const
  {
    transcendental_fct_type_C* res = new transcendental_fct_type_C(x1,x2,i_num,i_denom,
								   ij_num,ij_denom,
								   pre_num,pre_denom,
   ...
   *
   */
  ex transcendental_fct_type_C::shift_plus_one(void) const
  {
    int k;
    ...
    lst new_ij_num = lst();
    for ( k=0;k<ij_num.nops();k++) new_ij_num.append( ij_num.op(k) + 1 );

    lst new_ij_denom = lst();
    for ( k=0;k<ij_denom.nops();k++) new_ij_denom.append( ij_denom.op(k) + 1 );

    return x1*create_transcendental_fct_type_C(x1,x2,
