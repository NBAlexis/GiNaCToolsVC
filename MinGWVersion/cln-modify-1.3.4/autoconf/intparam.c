...
typedef unsigned long long  _longptr;
#endif
...
#define check_sizeof_pointer(type,typestr)  \
  { if (!(sizeof(type) <= sizeof(_longptr)))                                 \
      printf("#error \"Type %s does not fit into a _longptr!!\"\n",typestr); \
  }
    ...
    { int i;                                                                                           \
      ...
      for (i = 0; i<100; i++)                                                                          \
        { ulong sample;                                                                                \
          ...
          differences1 |= ((_longptr)sample1 ^ (_longptr)(type1)(sample2));                                  \
          differences2 |= ((_longptr)sample2 ^ (_longptr)(type2)(sample1));                                  \
        }                                                                                              \
      if (differences1==0)                                                                             \
...
int main()
{ 
	printf("/* size of long is 4 bytes in MinGW64, use long long instead of long for pointer */\n");
	printf("#define __MinGW64__\n");
  printf("\n");
